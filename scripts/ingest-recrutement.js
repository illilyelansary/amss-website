// scripts/ingest-recrutements.js
// Usage Netlify (pré-build) :
//   node scripts/ingest-recrutements.js content/recrutements src/data/recrutementData.auto.js public/avis
...
function pickDomaine(text) {
  ...
  if (/agri|pastor|securite alimentaire|sécurité alimentaire|subsistance/.test(t)) return 'Sécurité Alimentaire'
...
async function processPdf(file, publicDir) {
  ...
  if (mObj) description = mObj[1]
  else description = text.slice(0, 600)
  description = description.replace(/\n{2,}/g, ' ').replace(/\s{2,}/g, ' ').trim()

  let competences = []
  const mComp = text.match(/(?:profil\s+recherch[ée]|comp[ée]tences?)\s*[:\n]+([\s\S]{100,1200})/i)
  if (mComp) competences = extractBulletList(mComp[1])
...
async function main() {
  ...
  for (const f of pdfs) {
    try {
      ...
      if (expired) archives.push({ ...record, statut: 'Clôturé', dateCloture: record.dateExpiration })
      else enCours.push(record)
    } catch (e) {
  ...
  const banner =
`// Fichier généré automatiquement par scripts/ingest-recrutements.js
// Généré le: ${new Date().toISOString()}
// NE PAS ÉDITER À LA MAIN — déposez/maj vos PDFs dans content/recrutements/
`
  const body =
`${banner}
