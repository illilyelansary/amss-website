...
function ProjectCard({ p, ended = false }) {
  const statusColor = ended ? 'text-blue-600' : p.usaidNote ? 'text-red-600' : 'text-green-600'
  const dot = ended ? 'bg-blue-500' : p.usaidNote ? 'bg-red-500' : 'bg-green-500'
  return (
      ...
      <div className="space-y-2 text-sm">
        ...
        {'beneficiaries' in p && (
          <div className="flex justify-between">
            ...
            <span className="font-medium text-right">{fmt(p.beneficiaries)}</span>
          </div>
        )}
        {(p.domaine || p.domain) && (
          <div className="flex justify-between">
            <span className="text-muted-foreground">Domaine :</span>
            <span className="font-medium text-right">{p.domaine || p.domain}</span>
          </div>
        )}
      </div>
    </article>
...
export default function SantePage() {
  // Projets Santé (en cours & terminés) filtrés via domaine canon
  ...
  const totalBenefEnCours = useMemo(
    () => santeEnCours.reduce((acc, p) => acc + (Number(p.beneficiaries || 0) || 0), 0),
    [santeEnCours]
  )
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-xl text-muted-foreground leading-relaxed">
              Accès aux soins essentiels, santé de la reproduction, vaccination et lutte contre la malnutrition.
            </p>
          ...
          <div className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="bg-white rounded-xl p-5 shadow-sm border">
              ...
              <div className="text-2xl font-semibold mt-1 flex items-center gap-2">
                <Users className="h-5 w-5" /> {santeEnCours.length}
              </div>
            </div>
            <div className="bg-white rounded-xl p-5 shadow-sm border">
              <div className="text-sm text-muted-foreground">Projets terminés</div>
              <div className="text-2xl font-semibold mt-1 flex items-center gap-2">
                <CheckCircle className="h-5 w-5" /> {santeTermines.length}
              </div>
            </div>
            <div className="bg-white rounded-xl p-5 shadow-sm border">
              <div className="text-sm text-muted-foreground">Bénéficiaires (en cours)</div>
              <div className="text-2xl font-semibold mt-1 flex items-center gap-2">
                <TrendingUp className="h-5 w-5" /> {fmt(totalBenefEnCours)}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-12">
              <h2 className="text-3xl font-bold text-foreground mb-4">Nos Interventions Santé</h2>
              <p className="text-xl text-muted-foreground">Des approches intégrées et centrées sur les populations</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              <div className="bg-white rounded-xl p-6 shadow-sm border hover:shadow-lg transition-shadow">
                ...
                <h3 className="text-lg font-semibold mb-2">SR/PF & Santé Maternelle</h3>
                <p className="text-sm text-muted-foreground">Accès à la SR/PF, CPN/CPN, accouchements assistés et soins postnataux.</p>
              </div>
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm border hover:shadow-lg transition-shadow">
                ...
                <h3 className="text-lg font-semibold mb-2">Nutrition Communautaire</h3>
                <p className="text-sm text-muted-foreground">Prévention/PEC de la malnutrition, UNAs, démonstrations culinaires.</p>
              </div>
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm border hover:shadow-lg transition-shadow">
                ...
                <h3 className="text-lg font-semibold mb-2">Prévention & Vaccination</h3>
                <p className="text-sm text-muted-foreground">Campagnes de vaccination, sensibilisation, chaîne du froid.</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div>
                ...
                <div className="space-y-6">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h3 className="font-semibold">Santé maternelle</h3>
                      <p className="text-muted-foreground">2 500+ femmes bénéficiaires de CPN</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-4">
                    <div className="bg-orange-100 p-3 rounded-lg"><Activity className="h-6 w-6 text-orange-600" /></div>
                    <div>
                      <h3 className="font-semibold">Nutrition infantile</h3>
            ...
            {santeTermines.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {santeTermines.map((p, i) => (
                  <ProjectCard key={`s-out-${i}`} p={p} ended />
                ))}
              </div>
            )}
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              <div>
                <h2 className="text-3xl font-bold text-foreground mb-6">Approche Intégrée Santé–Nutrition</h2>
                <p className="text-muted-foreground mb-6 leading-relaxed">
                  Prévention, prise en charge, référencement et mobilisation communautaire pour un impact durable.
                </p>
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          <div className="max-w-2xl mx-auto text-center">
            <h2 className="text-3xl font-bold text-foreground mb-6">Soutenez la Santé au Sahel</h2>
