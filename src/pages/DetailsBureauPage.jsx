...
import { MapPin, Phone, Mail, Building, Users, Globe, ArrowLeft } from 'lucide-react'
...
export default function DetailsBureauPage() {
  ...
  const bureau = useMemo(() => {
    ...
    return bureaux.find(b => b.slug === s) || bureaux[0]
  }, [slug])
      ...
      <section className="py-12 bg-gradient-to-br from-primary/10 to-accent/10">
        <div className="container mx-auto px-4">
          <div className="max-w-5xl mx-auto flex items-center justify-between">
            <h1 className="text-3xl md:text-4xl font-bold text-foreground">{bureau.ville}</h1>
            <Button asChild variant="outline"><Link to="/contact"><ArrowLeft className="h-4 w-4 mr-2" />Retour</Link></Button>
          </div>
          <p className="max-w-5xl mx-auto text-muted-foreground mt-2">{bureau.type}</p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-5xl mx-auto">
            <div className="lg:col-span-2 rounded-xl overflow-hidden border border-border shadow-sm">
              <iframe
                ...
                src={`https://www.google.com/maps?q=${encodeURIComponent(bureau.coordsQuery || bureau.ville + ' Mali')}&output=embed`}
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-border space-y-4 text-sm text-muted-foreground">
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {(bureau.telephones || []).map((t, i) => (
                      <div key={i}><a className="hover:underline" href={`tel:${t.replace(/\s+/g,'')}`}>{t}</a></div>
                    ))}
                  </div>
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {bureau.emails.map((e, i) => (
                        <a key={i} className="hover:underline" href={`mailto:${e}`}>{e}</a>
                      ))}
                    </div>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {bureau.zones.map((z, i) => (
                        <span key={i} className="inline-block bg-primary/10 text-primary px-2 py-1 rounded text-xs">{z}</span>
                      ))}
                    </div>
                  </div>
                </div>
              ...
              {bureau.siteWeb && (
                <div><a className="hover:underline" target="_blank" rel="noreferrer" href={bureau.siteWeb}>{bureau.siteWeb}</a></div>
              )}

              <div className="pt-2">
                <Button asChild className="w-full"><Link to="/contact">Contacter ce bureau</Link></Button>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-8 bg-muted/30">
        <div className="container mx-auto px-4">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-2xl font-semibold text-foreground mb-4">Partenaires/Bailleurs intervenant dans la zone</h2>
            {partners.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {partners.map((p, i) => (
                  <span key={i} className="inline-block bg-white border border-border rounded-lg px-3 py-1 text-sm text-foreground shadow-sm">{p}</span>
                ))}
              </div>
            ) : (
