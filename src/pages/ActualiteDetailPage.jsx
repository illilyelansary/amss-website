...
import { Calendar, Tag, MapPin, ArrowRight, Share2, ChevronLeft } from 'lucide-react'
import { actualites } from '../data/actualitesData'

const ActualiteDetailPage = () => {
  const { id } = useParams()
  const navigate = useNavigate()
  const actu = actualites.find(a => String(a.id) === String(id))
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="max-w-4xl mx-auto mt-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {actualites
                .filter(a => a.id !== actu.id)
                .slice(0, 4)
                .map(a => (
                  <Link to={`/actualites/${a.id}`} key={a.id} className="flex gap-3 items-center group">
