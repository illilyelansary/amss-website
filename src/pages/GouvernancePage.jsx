...
import { Users, Landmark, Handshake, Megaphone, Scale, TrendingUp, MapPin, ArrowRight, CheckCircle, Clock } from 'lucide-react'
...
export default function GouvernancePage() {
  const govEnCours = useMemo(() => (projetsEnCours || []).filter(p => hasCanon(p, CANON_KEY)), [])
  const govTermines = useMemo(() => (projetsTermines || []).filter(p => hasCanon(p, CANON_KEY)), [])

  const totalBenef = useMemo(
    () => govEnCours.reduce((acc, p) => acc + Number(p.beneficiaries || 0), 0),
    [govEnCours]
  )
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="py-20 bg-gradient-to-br from-emerald-50 to-green-100">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            <Landmark className="h-16 w-16 text-emerald-700 mx-auto mb-6" />
            <h1 className="text-4xl md:text-5xl font-bold text-foreground mb-6">
              Gouvernance & Paix
            </h1>
            <p className="text-xl text-muted-foreground leading-relaxed">
              Participation citoyenne, cohésion sociale, redevabilité et médiation pour des institutions plus fortes.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 bg-white/60 border-y">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-5xl mx-auto">
            <Stat icon={<Users className="h-5 w-5" />} label="Projets en cours" value={govEnCours.length} />
            <Stat icon={<CheckCircle className="h-5 w-5" />} label="Projets terminés" value={govTermines.length} />
      ...
      <section className="py-16">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            <Card title="Cohésion sociale" icon={<Handshake className="h-7 w-7 text-emerald-700" />}>
              Cadres de dialogue, médiation communautaire, prévention des conflits.
            </Card>
            <Card title="Participation citoyenne" icon={<Megaphone className="h-7 w-7 text-green-700" />}>
              Budgets participatifs, consultation publique, mécanismes de redevabilité.
            </Card>
            <Card title="Renforcement institutionnel" icon={<Landmark className="h-7 w-7 text-teal-700" />}>
              Capacitation des collectivités et services techniques, gouvernance locale.
            </Card>
            <Card title="État de droit" icon={<Scale className="h-7 w-7 text-lime-700" />}>
              Accès à la justice, appui aux acteurs juridiques et coutumiers.
            </Card>
          </div>
        </div>
      </section>
      ...
      <section className="py-16">
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          {govEnCours.length === 0 ? (
            <p className="text-muted-foreground">Aucun projet en cours en Gouvernance & Paix.</p>
          ) : (
      ...
      <section className="py-16 bg-muted/30">
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          {govTermines.length === 0 ? (
            <p className="text-muted-foreground">Aucun projet terminé en Gouvernance & Paix.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {govTermines.map((p, i) => (
      ...
      <section className="py-16 bg-white/60 border-y">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Kpi title="Approche inclusive" icon={<Users className="h-5 w-5" />}>
              Participation des femmes et jeunes, OSC locales et leaders communautaires.
            </Kpi>
            <Kpi title="Zones d’intervention" icon={<MapPin className="h-5 w-5" />}>
              Tombouctou • Gao • Mopti • Kidal • Ségou
            </Kpi>
            <Kpi title="Partenaires" icon={<Users className="h-5 w-5" />}>
              PTF, Ministères, Collectivités, réseaux de paix
            </Kpi>
          </div>
...
function Card({ title, icon, children }) {
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm border border-border hover:shadow-lg transition-shadow">
      <div className="bg-emerald-50 w-16 h-16 rounded-lg flex items-center justify-center mb-4">{icon}</div>
