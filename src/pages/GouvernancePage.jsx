...
const filterProjectsForGovernance = (list) =>
  (list || []).filter(p => splitDomains(p.domain).some(isGovernanceDomain))

const ProjectCard = ({ p, ended = false }) => {
  const start = p?.startDate ? new Date(p.startDate) : null
  const end = p?.endDate ? new Date(p.endDate) : null
  const fmt = (d) => (d && !isNaN(d)) ? d.toLocaleDateString('fr-FR', { year: 'numeric', month: 'short' }) : 'N/D'

  return (
    <article className="bg-white rounded-xl p-6 shadow-sm border border-border hover:shadow-md transition-shadow">
      <div className="flex items-center gap-2 mb-3">
        <div className={`w-2.5 h-2.5 rounded-full ${ended ? 'bg-blue-500' : (p.usaidNote ? 'bg-red-500' : 'bg-green-500')}`} />
        <span className={`text-xs font-medium ${ended ? 'text-blue-600' : (p.usaidNote ? 'text-red-600' : 'text-green-600')}`}>
      ...
      <div className="space-y-2 text-xs md:text-sm">
        <div className="flex items-start gap-2">
          ...
          <div><span className="text-muted-foreground">Région : </span><span className="font-medium">{p.region || 'N/D'}</span></div>
        </div>
        <div className="flex items-start gap-2">
          <BadgeCheck className="h-4 w-4 text-muted-foreground mt-0.5" />
          <div><span className="text-muted-foreground">Bailleur : </span><span className="font-medium">{p.donor || 'N/D'}</span></div>
        </div>
        <div className="flex items-start gap-2">
